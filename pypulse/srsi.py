...
class SRSI(PulseBasic):
    ...
    def __init__(self, folder_path: str, mode_acquire: str, wavelength_center: float, 
                 wavelength_width: float, n_omega: int, n_fft: int, n_iteration: int, 
        ...
        self.__read_spectrum_from_txt(folder_path, mode_acquire)
        self.Sw_interference = self.resample_signal(self.Sw_interference, wavelength_center, n_omega, wavelength_width, method)
        
        match mode_acquire:
            case "double":
                self.Sw_unknown = self.resample_signal(self.Sw_unknown, wavelength_center, n_omega, wavelength_width, method)
            case "triple":
                self.Sw_unknown = self.resample_signal(self.Sw_unknown, wavelength_center, n_omega, wavelength_width, method)
                self.Sw_reference = self.resample_signal(self.Sw_reference, wavelength_center, n_omega, wavelength_width, method)
        ...
        self.retrieve_phase(n_iteration)
        
    def __read_spectrum_from_txt(self, folder_path: str, mode_acquire: str) -> None:
        """
        ...
        match mode_acquire:
            ...
            case "triple":
                ...
                self.Sw_reference = np.loadtxt(matching_file[0], delimiter='\t', skiprows=14, encoding='iso-8859-1')[:, 1]
                
    def retrieve_phase(self, n_iteration: int) -> None:
        """
