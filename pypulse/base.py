...
class PulseBasic(ABC):
    ...
    def iFt(Ew: np.ndarray, n_omega: int, n_fft: int) -> np.ndarray:
        ...
        Et = fftshift(ifft(fftshift(Ew, axes=2), axis=2), axes=2)
        return Et

    @staticmethod
    def F(Exy: np.ndarray) -> np.ndarray:
        EXY = ifftshift(ifftshift(fft2(fftshift(fftshift(Exy, axes=1), axes=0)), axes=1), axes=0)
        return EXY

    @staticmethod
    def iF(EXY: np.ndarray) -> np.ndarray:
        Exy = ifftshift(ifftshift(ifft2(fftshift(fftshift(EXY, axes=1), axes=0)), axes=1), axes=0)
        return Exy
    ...
    def resample_signal(
        ...
        idx = np.where(np.abs(self.get_property("wavelength") - wavelength_center) > wavelength_width / 2)[0]
        ...
        self.wavelength_axis = 2 * np.pi * self.SPEED_OF_LIGHT / (self.omega_axis + self.get_property("omega_center"))
        spectrum_resampled = interp.interp1d(omega, spectrum, kind=method, bounds_error=False, fill_value=0)(
            self.omega_axis
        )
    ...
    def fourier_transform_spectral_interferometry(
        ...
        f = (np.max(self.omega_axis) + (n_fft - n_omega) / 2 * np.abs(self.omega_axis[1] - self.omega_axis[0])) / np.pi
        ...
        if delay_min is None:
            ...
            for i in range(len(self.get_property("row"))):
                ...
                if len(idx_peaks) > 1:
                    delay[self.get_property("row")[i], self.get_property("col")[i]] = t_axis_temp[
                        idx_peaks[np.argsort(signal[idx_peaks])[-1]]
                    ]
        else:
            t_axis_temp = self.t_axis[self.t_axis > delay_min / 2]
            for i in range(len(self.get_property("row"))):
                signal = rescale(np.abs(St[self.get_property("row")[i], self.get_property("col")[i], :]))
                signal = signal[self.t_axis > delay_min / 2]
                idx_peaks, _ = find_peaks(signal, height=0.01)
                if len(idx_peaks) > 1:
                    delay[self.get_property("row")[i], self.get_property("col")[i]] = t_axis_temp[
                        idx_peaks[np.argsort(signal[idx_peaks])[-1]]
        ...
        filter_AC = np.exp(
            -(
                ((self.t_axis.reshape(1, 1, -1) - delay[:, :, np.newaxis]) / filter_width[:, :, np.newaxis])
                ** filter_order
            )
        )
        filter_DC = np.exp(-(((self.t_axis.reshape(1, 1, -1)) / filter_width[:, :, np.newaxis]) ** filter_order))
        ...
        phase = np.angle(Sw_AC * np.exp(1j * self.omega_axis.reshape(1, 1, -1) * delay[:, :, np.newaxis]))
